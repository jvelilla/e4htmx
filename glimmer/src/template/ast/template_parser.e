...
feature -- Parsing
	...
	parse (a_template: READABLE_STRING_GENERAL): ARRAYED_LIST [TEMPLATE_NODE]
		...
		local
			...
			l_raw: BOOLEAN
			l_val_end: INTEGER
			l_text_node: TEXT_NODE
			l_var_node: VARIABLE_NODE
		do
			last_error := Void
...
feature {NONE} -- Parser State
	...
	active_list: ARRAYED_LIST [TEMPLATE_NODE]
			-- Currently active list being compiled into

	stack: ARRAYED_LIST [ARRAYED_LIST [TEMPLATE_NODE]]
			-- Stack of active lists

	node_stack: ARRAYED_LIST [TEMPLATE_NODE]
...
feature {NONE} -- Tag Processing
	...
	process_block_tag (a_tag: STRING_32)
		...
		local
			...
			l_cond_node: CONDITIONAL_NODE
			l_loop_node: LOOP_NODE
			l_sec_node: SECTION_NODE
			l_y_node: YIELD_NODE
			l_inc_node: INCLUDE_NODE
			false_branch: ARRAYED_LIST [TEMPLATE_NODE]
		do
			...
			elseif a_tag.starts_with ("each ") then
				...
				if l_parts.count = 2 then
					...
					create l_loop_node.make (l_iterator, l_collection, create {ARRAYED_LIST [TEMPLATE_NODE]}.make (10))
			...
			elseif a_tag.starts_with ("section ") then
				...
				create l_sec_node.make (l_sec_name, create {ARRAYED_LIST [TEMPLATE_NODE]}.make (10))
