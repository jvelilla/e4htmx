...
feature -- Parsing
	...
	parse (expression: STRING_32): GLM_EXPRESSION_NODE
		...
		local
			...
			l_and_nodes: ARRAYED_LIST [GLM_EXPRESSION_NODE]
			l_or_nodes: ARRAYED_LIST [GLM_EXPRESSION_NODE]
			i: INTEGER
		do
			...
			if l_expr.has_substring (" and ") then
				...
				create {GLM_AND_EXPRESSION_NODE} Result.make (l_and_nodes)
			elseif l_expr.has_substring (" or ") then
				...
				create {GLM_OR_EXPRESSION_NODE} Result.make (l_or_nodes)
			elseif l_expr.starts_with ("not ") then
				Result := create {GLM_NOT_EXPRESSION_NODE}.make (parse (l_expr.substring (5, l_expr.count)))
			elseif l_expr.starts_with ("exists ") then
				...
				create {GLM_EXISTS_EXPRESSION_NODE} Result.make (l_left_str)
			else
				l_operator := find_operator (l_expr)
				if l_operator /= Void then
					l_parts := split_string (l_expr, l_operator)
					if l_parts.count = 2 then
						create {GLM_COMPARISON_EXPRESSION_NODE} Result.make (l_parts.first, l_parts.last, l_operator)
					else
						create {GLM_VARIABLE_EXPRESSION_NODE} Result.make (l_expr)
					end
				else
					create {GLM_VARIABLE_EXPRESSION_NODE} Result.make (l_expr)
				end
