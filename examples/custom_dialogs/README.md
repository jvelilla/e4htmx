...
.
├── src/
│ ├── htmx_custom_dialogs.e         # Main application service
│ └── htmx_custom_dialogs_execution.e # Request execution handling
├── www/
│ ├── index.html          # Main HTML file
│ └── styles.css          # Application styles
└── htmx_custom_dialogs.ecf         # Eiffel configuration file
```
## Overview
...
Our demo currently demonstrates HTMX's custom_dialogs feature:

1. **Page custom_dialogs**: Using `hx-boost` attribute to enhance regular links:
   ```html
...
1. Compile the project:
   ```bash
   ec -config htmx_custom_dialogs.ecf
   ```
